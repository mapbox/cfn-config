...
function parse(args, env) {
  ...
  // Handle the kms arg
  if (cli.flags.kms && typeof cli.flags.kms === 'boolean') cli.flags.kms = 'alias/cloudformation';

  return {
    command: cli.input[0],
    environment: cli.input[1],
    templatePath: cli.input[2] ? path.resolve(cli.input[2]) : undefined,
    overrides: { force: cli.flags.force },
    options: cli.flags,
    help: cli.help
  };
}
...
function main(parsed, callback) {
  ...
  if (parsed.command === 'info') args.push(parsed.options.extended);

  if (/create|update/.test(parsed.command)) args.push(parsed.templatePath);

  if (parsed.options.kms && parsed.command === 'save') args.push(parsed.options.kms);
