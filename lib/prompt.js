...
class Prompt {
     ...
     * @param {boolean} [defaultValue=true] - boolean to use as default. if omitted, defaults to true (yes)
     * @param {function} callback - a function fired with the user's response as a boolean
     */
    static confirm(message, defaultValue, callback) {
        if (!callback) {
     ...
     */
    static input(message, def, callback) {
     ...
     */
    static configuration(configs, callback) {
        inquirer.prompt({
            type: 'list',
            name: 'config',
     ...
     */
    static parameters(questions, callback) {
