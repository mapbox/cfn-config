...
var operations = module.exports.operations = {
  ...
  promptParameters: function(context) {
    ...
    if (context.overrides.defaultConfig) {
      var s3Url = context.overrides.defaultConfig;
      lookup.defaultConfiguration(s3Url, function (err, info) {
        if (err) return (err);
        Object.keys(info).forEach(function(key) {
          if (newTemplateParameters[key] || context.oldParameters[key])
            context.oldParameters[key] = context.overrides.defaultConfig[key];
        })
      });
    }
