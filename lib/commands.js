...
module.exports = function(config) {
  ...
  commands.update = function(suffix, templatePath, overrides, callback) {
    ...
    console.log('overrides.skipConfirmParameters should be true');
    console.log(overrides.skipConfirmParameters);
    console.log('overrides.skipConfirmTemplate should be true');
    console.log(overrides.skipConfirmTemplate);
    var operationsArray = [operations.updatePreamble, operations.getMasterConfig];

    // if (!overrides.skipConfirmParameters)
    operationsArray = operationsArray.concat([operations.promptParameters, operations.confirmParameters]);
    if (!overrides.skipConfirmTemplate)
      operationsArray = operationsArray.concat([operations.confirmTemplate]);

    operationsArray = operationsArray.concat([
      operations.mergeMetadata,
      operations.saveTemplate,
      operations.validateTemplate,
    ...
    console.log('operationsArray.length should be 11');
...
var operations = module.exports.operations = {
  ...
  promptParameters: function(context) {
    ...
    if (context.overrides.force) {
      ...
      console.log(context.newParameters);
      return context.next();
    }

    if (context.overrides.skipConfirmParameters) {
      context.newParameters = {};
      Object.keys(newTemplateParameters).forEach(function(key){
        if (context.oldParameters[key] !== undefined) {
          context.newParameters[key] = context.oldParameters[key];
  ...
  confirmParameters: function(context) {
    ...
    if (context.overrides.skipConfirmParameters) return context.next();
