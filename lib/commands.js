...
module.exports = function(config) {
  ...
  commands.create = function(suffix, templatePath, callback) {
  ...
  commands.update = function(suffix, templatePath, force, callback) {
  ...
  commands.delete = function(suffix, callback) {
    var context = module.exports.commandContext(config, suffix, [
  ...
  commands.info = function(suffix, callback) {
    var stackName = [config.name, suffix].join('-');
    lookup.info(stackName, config.region, callback);
  };
...
module.exports.commandContext = function(config, suffix, operations, callback) {
  ...
  var context = {
    ...
    suffix: suffix,
    stackName: [config.name, suffix].join('-'),
...
var operations = module.exports.operations = {
  updatePreamble: function(context) {
    ...
    preamble.defer(template.read, context.templatePath);
  ...
  confirmTemplate: function(context) {
    ...
    if (context.force) {
      process.stdout.write(['', 'Template has changed:', diff, ''].join('\n'));
      return context.abort(new Error('Cannot change template with --force'));
    }
  ...
  loadConfig: function(context) {
    if (!context.configName) return context.next();

    lookup.configuration(context.baseName, context.configBucket, context.configName, function(err, info) {
      if (err) {
        var msg = '';
        if (err instanceof lookup.BucketNotFoundError) msg += 'Could not find config bucket: ';
  ...
  confirmSaveConfig: function(context) {
    ...
    prompt.confirm('Save this configuration as "' + context.suffix + '"?', function(err, ready) {
      if (!ready) return context.abort();
      context.next();
    });
  },
