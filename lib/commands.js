...
module.exports = function(config) {
  ...
  commands.info = function(suffix, resources, callback) {
    ...
    lookup.info(stackName, config.region, resources, callback);
  };

  commands.save = function(suffix, overrides, callback) {
    if (typeof overrides === 'function') {
      callback = overrides;
      overrides = {};
    }
    ...
    context.overrides = overrides;
    context.overrides.savedName = context.overrides.savedName || suffix;
    context.next();
  };
...
var operations = module.exports.operations = {
  ...
  selectConfig: function(context) {
    ...
    prompt.configuration(context.configNames, function(err, selectedConfig) {
      if (selectedConfig === 'New configuration') return context.next();

      context.configName = selectedConfig;
      context.next();
    });
  },
  ...
  confirmSaveConfig: function(context) {
    ...
    prompt.confirm('Save this configuration as "' + context.overrides.savedName + '"?', function(err, ready) {
      if (!ready) return context.abort();
      context.next();
    });
  },
