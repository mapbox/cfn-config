      ...
      Object.keys(masterConfigJSON).forEach(function(key) {
        if (context.oldParameters.hasOwnProperty(key)) {
          if (context.oldParameters[key].indexOf('secure') > -1) {
            // within old parameters, look at if it's secure
            var kms = new AWS.KMS({
              region: context.overrides.templateOptions.AWS_DEFAULT_REGION,
              maxRetries: 10
            });
            // console.log(context.oldParameters[key].replace(/^secure:/,''))
            var key = context.oldParameters[key].replace(/^secure:/,'');
            kms.decrypt(
            {
              CiphertextBlob: new Buffer(key, 'base64')
            }, function (err, data) {
              console.log('decrypted below')
              console.log(new Buffer(data.Plaintext, 'base64').toString('utf-8'))
              context.oldParameters[key] = new Buffer(data.Plaintext, 'base64').toString('utf-8');
              console.log(context.oldParameters[key] === masterConfigJSON[key])
