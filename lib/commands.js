...
var operations = module.exports.operations = {
  updatePreamble: function(context) {
    ...
    if (context.overrides.masterConfig) {
      ...
      preamble.defer(lookup.defaultConfiguration, s3Url);
    }

    preamble.await(function(err, templateBody, oldParams, oldTemplate, masterConfig) {
      if (err) {
        var msg = '';
      ...
      if (context.overrides.masterConfig) {
        Object.keys(masterConfig).forEach(function(key) {
          if (err) return context.abort(err);
          if (context.oldParameters[key])
            context.oldParameters[key] = masterConfig[key];
        });
      }
      context.next();
    });
